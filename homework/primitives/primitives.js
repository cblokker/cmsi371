...
var Primitives = {
    ...
    lineDDA: function (context, x1, y1, x2, y2, dashLength, dashBreak, color) {
        ...
        color = color || [0, 0, 0];
        for (i = 0; i <= steps; i += 1) {
            if (i % (dashBreak + dashLength) < dashLength) {
    ...
    lineBres1: function (context, x1, y1, x2, y2, dashLength, dashBreak, color) {
        var x = x1,
            y = y1,
            dx = x2 - x1,
            dy = y1 - y2,
        ...
        color = color || [0, 0, 0];
        while (true) {
            if (x % (dashLength + dashBreak) < dashLength) {
                this.setPixel(context, x, y, color[0], color[1], color[2]);
            }
            if (x === x2) {
                return;
    ...
    lineBres2: function (context, x1, y1, x2, y2, dashLength, dashBreak, color) {
        var x = x1,
            y = y1,
            dx = x2 - x1,
            dy = y1 - y2,
    ...
    lineBres3: function (context, x1, y1, x2, y2, dashLength, dashBreak, color) {
        var x = x1,
            y = y1,
            dx = x2 - x1,
            dy = y1 - y2,
    ...
    lineBresenham: function (context, x1, y1, x2, y2, dashLength, dashBreak, color) {
        var x = x1,
            y = y1,
            dx = x2 - x1,
            dy = y1 - y2,
    ...
    plotCirclePoints: function (context, xc, yc, x, y, colorStart, colorEnd, r) {
        var n,
            ...
            colorDifB = colorStart[2] - colorEnd[2];

        this.setPixel(context, xc + x, yc + y, colorStart[0], colorStart[1], colorStart[2]);
        this.setPixel(context, xc + x, yc - y, colorStart[0], colorStart[1], colorStart[2]);
        ...
        for (i = xc - x, n = 0; i < xc + x ; i += 1, n += 1) {
            for (j = yc - y; j < yc + y; j += 1) {
                this.setPixel(context, i, j, colorStart[0] - n * colorDifR / (2 * r),
                                             colorStart[1] - n * colorDifG / (2 * r),
                                             colorStart[2] - n * colorDifB / (2 * r));
            }
        }
    },

    // First, the most naive possible implementation: circle by trigonometry.
    circleTrig: function (context, xc, yc, r, colorStart, colorEnd) {
        var theta = 1 / r,
        ...
        while (x >= y) {
            this.plotCirclePoints(context, xc, yc, x, y, colorStart, colorEnd, r);
            x = x * c - y * s;
            y = x * s + y * c;
        }
    },
